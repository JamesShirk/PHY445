...
def eff():
    ...
    # Inches!!!
    domega = (1.5**2 * np.pi) / ((11**2))

    Energy = np.array([10, 50, 100, 150, 200, 250, 300, 350, 400, 500, 600, 700, 800, 900, 1000])
    Efficiency = np.array([1, 1, 1, 1, 1, 0.99, 0.97, 0.96, 0.95, 0.93, 0.90, 0.87, 0.85, 0.83, 0.81]) * domega
    ...
    if __name__ == "__main__":
        ...
        x = np.linspace(10, 1200)
        #plt.plot(x, interp_p2t(x)*interp_eff(x)*domega)
        plt.plot(x, interp_eff(x)*domega)
        plt.xlabel("E (KeV)")
        plt.ylabel(r"$\varepsilon d \Omega$")
        plt.grid()
        plt.savefig("eff.png")
        plt.close()
